...
task :new_post, :title do |t, args|
  ...
  open(filename, 'w') do |post|
    ...
    post.puts "description: <A SHORT DESCRIPTION OF THE POST <= 200 CHARACTERS >"
    post.puts "longdescription: <A LONG DESCRIPTION OF THE POST BETWEEN 230 AND 320 CHARACTERS>"
    post.puts "date: #{Time.now.strftime('%Y-%m-%d %H:%M')}"
    post.puts "category: <FROM HERE: https://docs.google.com/spreadsheets/d/1e_RKzi8kVwzqPG8si8kyDOWPiBk9tI-XNGh0KgRIF7Q>"
    post.puts "banner:"
    post.puts "  text: <TEXT TO BE USED AS A BANNER>"
    post.puts "author:"
    post.puts "  name: <YOUR NAME>"
    post.puts "  url: <YOUR URL>"
    post.puts "  mail: <YOUR MAIL>"
    post.puts "  avatar: <LINK TO PROFILE PIC>"
    post.puts "design:"
