...
**TL;DR:**  In this tutorial, I'll show you how to leverage Loopback to build out your REST APIs quickly. Check out the [repo](https://github.com/auth0-blog/preact-authentication-tutorial) to get the code.

---

## RESTful API Overview

API (Application Programming Interface) endpoints are the connections between your application and the rest of the developer community. You can decide to open up your data source to the world by crafting API endpoints that developers can consume. Furthermore, developing application that will require clients for different platforms such as Desktop, Android, iOS and windows platform will most likely need a RESTful API for all the clients to access data seamlessly, unless of course you are engaging GraphQL, the alternate option to RESTful APIs.

If you have built a RESTful API from scratch before now, you'll understand that there are a lot of processes you have to follow, a lot of code to write, and a lot of testing to do to ensure your APIs work correctly and are designed to become very robust as painless as possible. Loopback was designed to fast-track those processes and take away the pain of developing RESTful APIs from scratch.
...
* Allowing a developer also create dynamic models in the absence of a schema.
* Providing SDKs for iOS, Android and AngularJS to easily create client apps.

## Application Overview

In this tutorial, we'll build a RESTful API for a Star Wars application. A Star Wars API, **SWAPI** already exist. Rather than crafting a new API, we'll build a clone of the API. The objective of building the API clone is to learn how to craft APIs quickly with Loopback. Consider a case where you are asked to design an API for a new interesting HBO show, e.g Game of Thrones that has caught the attention of Google & Facebook.
...
5. **Species** - This resource is about the type of character in the Star Wars Universe.
...
We'll create all the models needed for this API. As mentioned earlier, we have 6 resources, `People`, `Films`, `Starships`, `Species` and `Planets`. In our API, we'll deal with just 5.
...
Next, select `common` model. Selecting this ensures that the models are stored in the `common` directory. You can also decide to store them in the `server` directory only.
...
* Property name: eye_color, Property type: string, Required? Yes
* Property name: name, Property type: string, Required? Yes

Repeat the same process for the `Films`, `Vehicles`, `Starships`, `Species` and `Planets` models with these properties:
...
Next, test the API. Run the command below in your terminal:
...
Let's make a POST request to the `/api/film` endpoint using the explorer.
...
Voila! Be careful, our API is not connected to any data source yet. Therefore, only one record can exist at a time. Creation of a new record will override the existing record.
...
The connection details can be found at `server/datasources.json`. Feel free to edit anytime.
...
Let's go with the second option. It's more effective. And we need test data. We'll write an automigration script.
...
This bunch of code simply creates the tables and seeds them with the data provided. Whenever your app is initialzied, this script will run and ensure the tables exist and are seeded with the right data.
...
The API receives a request with a JWT, the first thing to do is to validate the token. This consists of a series of steps, and if any of these fails then, the request must be rejected. The following list shows the validation steps needed:
...
Head over to your terminal and install the following:
...
In the code above, we required `express-jwt` and `jwks-rsa` modules. The `authCheck` variable does the check to validate the access tokens that are sent as headers. It validates the `audience`, `issuer` and algorithm used to sign the token.
...
**Loopback** is a great Node.js framework to design and build your APIs quickly as shown in this tutorial. It is a project backed up by IBM and the Strongloop team. They are committed to maintaining and improving on this amazing open source project.
