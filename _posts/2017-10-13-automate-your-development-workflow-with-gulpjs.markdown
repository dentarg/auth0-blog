...
- 2017-05-02-glossary-of-modern-javascript-concepts-part-2
---
...
* **Gulp**: Install Gulp globally in your terminal so that the `gulp` command can be run from any directory.
...
In a typical project, gulp needs some form of instructions to guide it in automating your tasks. These instructions are written in a file, called the `gulpfile`. Let's go ahead and create this file.
...
![Run common task](https://cdn.auth0.com/blog/gulp/run-common-task.png)
...
* Compress all our JavaScript files into just one file. This will make sure our app makes one HTTP request while serving JavaScript rather than multiple HTTP requests.
...
## Project Lingo - Install Gulp Plugins
...
A quick breakdown of what each of these plugins aim to accomplish in _Project Lingo_.

* **gulp-imagemin** - Minify PNG, JPEG, GIF and SVG images in _Project Lingo_.
* **gulp-less** - Compile Less files to CSS.
* **gulp-jshint** - Automatically detect errors and problems in Project Lingo's JavaScript code. Lint it!
* **gulp-uglify** - Minify JavaScript files in _Project Lingo_.
* **gulp-rename** - Rename a file.
* **gulp-concat** - Concatenate all the JavaScript files into one file in _Project Lingo_.
...
* **less task** - This task checks all the less files in our `less` directory, compiles them to CSS and copies them to a `dist/css` directory. If you use sass more often, there is a `gulp-sass` plugin available to compile your sass files to CSS.
    ...
    ```
* **scripts task** - This task checks all the JavaScript files in our `js` directory, concatenates them into a single file, `build.js`, copies the file into a `dist/js` directory, renames the file to `build.min.js` and minifies it.
